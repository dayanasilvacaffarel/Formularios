...
function validarInformacion() {
    ...
    if (!isNaN(obejetoInformacion.nombre) || obejetoInformacion.nombre.length < 3) {
        errores.push("El nombre debe tener al menos 3 caracteres")
    }

    if (obejetoInformacion.password.length < 6) {
        errores.push("La contraseña debe tener al menos 6 caracteres, entre letras y símbolos.")
    }

    if (obejetoInformacion.telefono.length < 10) {
        errores.push("No es un teléfono válido.")
    }


    if (document.querySelectorAll('input[type="checkbox"]:checked').length > 4) {
        errores.push("No se puede seleccionar más de 4 hobbies")
    }

    if (obejetoInformacion.nacionalidad == "") {
